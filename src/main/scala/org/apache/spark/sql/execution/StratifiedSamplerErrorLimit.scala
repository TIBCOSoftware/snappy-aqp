 ...
 */
final class StratifiedSamplerErrorLimit(_options: SampleOptions)
    extends StratifiedSampler(_options) with CastLongTime {

  def initCacheSize: Int = options.stratumSize
  def errorLimitColumn: Int = options.errorLimitColumn
  def errorLimitPercent: Double = options.errorLimitPercent
  ...
  private final class ProcessRows[U](val processFlush: (U, InternalRow) => U,
      val endBatch: U => U, rowEncoder: ExpressionEncoder[Row], var result: U)
      extends ChangeValue[Row, StratumReservoir] with CastDouble {

    override def keyCopy(row: Row): Row = row.copy()

    override val doubleColumnType: DataType = schema(errorLimitColumn).dataType
    ...
    def updateStratumStats(sc: StratumCacheWithStats, row: Row,
        updateSampleStats: Boolean, replacedRow: InternalRow): Unit = {
    ...
    override def mergeValueNoNull(row: Row, sr: StratumReservoir): (StratumCacheWithStats, Boolean,
        Boolean) = {
    ...
    override def mergeValue(row: Row,
      ...
      if (reservoirSize >= reservoirLen) {
        ...
        var replacedRow: InternalRow = EMPTY_ROW
        ...
        if (updateSampleStats) {
          ...
          reservoir(replaceIndex) = newMutableRow(row, rowEncoder)
          // update timeSlot start and end
          if (timeInterval > 0) {
        ...
        if ((scSize % 10) == 9) {
          ...
          if (sc.populationSize > (sc.sampleSize << 2) &&
            ...
            */
            ...
            val newReservoir = new Array[UnsafeRow](newReservoirSize)
        ...
        reservoir(reservoirSize) = newMutableRow(row, rowEncoder)
        sc.reservoirSize += 1
    ...
    override def segmentAbort(
        seg: SegmentMap[Row, StratumReservoir]): Boolean = {
      strata.synchronized {
  ...
  def checkCacheFlush(force: Boolean): java.lang.Boolean = {
    ...
    if (tableSize > (columnBatchSize * 10) && tableSize > (strata.size * 10)) {
  ...
  private def flushCache[U](force: Boolean, init: U,
    ...
    val batchSize = this.columnBatchSize
  ...
  override def append[U](rows: Iterator[Row],
      init: U, processFlush: (U, InternalRow) => U, startBatch: (U, Int) => U,
      endBatch: U => U, rowEncoder: ExpressionEncoder[Row], partIndex: Int): Long = {
    if (rows.hasNext) {
      val processedResult = new ProcessRows(processFlush, endBatch,
        rowEncoder, init)
  ...
  override def sample(items: Iterator[InternalRow],
    ...
    val sampleBuffer = new mutable.ArrayBuffer[InternalRow](batchSize / 10)
 ...
 */
private final class StratumCacheWithStats(_reservoir: Array[UnsafeRow],
