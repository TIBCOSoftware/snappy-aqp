...
There are two parameters (values) used when calculating the correct W and D dimensions of a count
min sketch, for the desired accuracy levels. The parameters are ε (epsilon) and δ (delta).
...
class CountMinSketch[T: ClassTag](val depth: Int, val width: Int, val seed: Int,
  ...
  private def this(depth: Int, width: Int, size: Long, hashA: Array[Long],
      table: Array[Array[Long]], confidence: Double) =
    this(depth, width, 0, CountMinSketch.initEPS(width), confidence, size, table, hashA)
  ...
  def add(item: T, count: Long): Long = {
    ...
    def matchItem(elem: Any): Long = {
      elem match {
        ...
        case s: UTF8String => addString(s.toString, count)
  ...
  def getIHashesFor(item: T, applyWidth: Boolean): Array[Int] = {
    def getHashes(elem: Any): Array[Int] = {
      elem match {
        ...
        case s: UTF8String => Filter.getHashBuckets(s.toString, depth, width, applyWidth)
        case num: Long =>
  ...
  def estimateCountAsApproximate(item: T): Approximate = {
    def matchItem(elem: Any): Approximate = {
      val count = elem match {
        case s: String => estimateCount(s)
        case s: UTF8String => estimateCount(s.toString)
        case num: Long => estimateCount(num)
        case num: Int => estimateCount(num)
        case num: Short => estimateCount(num)
        case num: Byte => estimateCount(num)
  ...
  def compress: CountMinSketch[T] = {
    ...
    if ((width & (width - 1)) != 0 || width < 2) {
      throw new CountMinSketch.CMSCompressException(
        "current width is not power of two: " + this.width)
    }
...
object CountMinSketch {
  ...
  //
  // Because of the actual types on which CMSFunctions operates (i.e. Int and Double),
  // the maximum number of decimal places should be 6.
  ...
  def read(s: DataInputStream): (Long, Int, Int, Double, Double, Array[Long],
      Array[Array[Long]]) = {
    try {
