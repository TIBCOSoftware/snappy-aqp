...
final class TopKHokusai[T](cmsParams: CMSParams, windowSize: Long,
    ...
    extends Hokusai[T](cmsParams, windowSize, epoch0, taAgg, iaAgg,
      intervalTracker, initialInterval, mBarInitializer) with TopK {
  ...
  def this(cmsParams: CMSParams, windowSize: Long,
    ...
    new mutable.MutableList[CountMinSketch[T]], new IntervalTracker(), 0, None)
  ...
  private val queryTillLastNTopK_Case3: (Int, Int, Int, Int,
    ...
    if (lastNIntervals > totalIntervals) {
      ...
      if (residualIntervals > (lengthOfLastInterval * 3) / 4) {
        ...
        unionedTopKKeys.foreach { item: T =>
          val count = this.taPlusIa.basicQuery(nearestPowerOf2Num.asInstanceOf[Int] + 1 to
              lastNIntervals, item, nearestPowerOf2Num.asInstanceOf[Int],
  ...
  private val queryTillLastNTopK_Case4: (Int, Int, Int, Int, Array[T]) => Array[(T, Approximate)] =
    (lastNIntervals: Int, totalIntervals: Int, n: Int, nQueried: Int, combinedTopKKeys: Array[T]) =>
      { // the number of intervals so far elapsed is not of form 2 ^n. So the time aggregates are
        ...
        // Identify the total range of intervals by identifying the highest 2^n , greater than or
        ...
        val skipLastInterval = if (computedIntervalLength > lastNIntervalsToQuery) {
          // this means that last one or many intervals lie within a time interval range drop
          // the last interval from the count of  best path as it needs to be handled separately
          bestPath.last
        } else {
          -1
        }
  ...
  private val combinedKeysTillLastNTopK_Case3: (Int, Int, Int, Int) =>
      Array[T] = (lastNIntervals: Int, totalIntervals: Int, n: Int, nQueried: Int) =>
    if (lastNIntervals > totalIntervals) {
  ...
  def getTopKTillTime(epoch: Long,
      combinedKeys: Array[T] = null): Option[Array[(T, Approximate)]] = {
    ...
    this.timeEpoch.timestampToInterval(epoch).flatMap(x => {
      ...
      val retVal = this.taPlusIa.queryLastNIntervals[Array[(T, Approximate)]](
  ...
  def getTopKForCurrentInterval: Option[Array[(T, Approximate)]] = {
    val retVal = this.mBar.asInstanceOf[TopKCMS[T]].getTopK
    if(retVal.length > 0) {
      Some(retVal)
    } else {
      None
  ...
  def getCombinedTopKKeysBetweenTime(epochFrom: Long,
    ...
    val retVal = this.getCombinedTopKKeysBetween(later, earlier)
    if(retVal.isEmpty) {
      None
    } else {
      Some(retVal)
  ...
  def getCombinedTopKKeysTillTime(epoch: Long): Option[Array[T]] = {
    ...
    this.timeEpoch.timestampToInterval(epoch).flatMap(x => {
      ...
      val retVal = this.taPlusIa.queryLastNIntervals[Array[T]](
        this.taPlusIa.convertIntervalBySwappingEnds(interval.asInstanceOf[Int]).asInstanceOf[Int],
  ...
  def getTopKBetweenTime(epochFrom: Long, epochTo: Long,
    combinedTopKKeys: Array[T] = null): Option[Array[(T, Approximate)]] =
    {
      ...
      val retVal = this.getTopKBetweenTime(later, earlier, combinedTopKKeys)
      if(retVal.length > 0) {
        Some(retVal)
      } else {
        None
  ...
  def getTopKBetweenTime(later: Int, earlier: Int,
      combinedTopKKeys: Array[T]): Array[(T, Approximate)] = {
    ...
    if(later == 0) {
      this.getTopKInCurrentInterval
    }
    else if (fromLastNInterval == 1 && tillLastNInterval == 1) {
      ...
      bestPath.foreach { interval =>
        ...
        if (start == taIntervalStartsAt && end == lengthTillInterval) {
          ...
          val topKCMS = this.taPlusIa.ta.aggregates(
            NumberUtils.isPowerOf2(interval) + 1).asInstanceOf[TopKCMS[T]]
  ...
  def getCombinedTopKKeysBetween(later: Int, earlier: Int): mutable.Set[T] = {
    ...
    val tillLastNInterval = this.taPlusIa.convertIntervalBySwappingEnds(earlier)
    if(later == 0) {
      this.mBar.asInstanceOf[TopKCMS[T]].getTopKeys
    }
    else if (fromLastNInterval == 1 && tillLastNInterval == 1) {
      val topKCMS = this.taPlusIa.ta.aggregates.head.asInstanceOf[TopKCMS[T]]
  ...
  def queryTimeAggregateForInterval(item: T, interval: Long): Approximate = {
    ...
    val topKCMS = this.taPlusIa.ta.aggregates(NumberUtils.isPowerOf2(interval) + 1)
        .asInstanceOf[TopKCMS[T]]
    topKCMS.getFromTopKMap(item).getOrElse(this.taPlusIa.queryTimeAggregateForInterval(item,
      interval))
  }
...
object TopKHokusai {
  ...
  def newZeroCMS[T: ClassTag](depth: Int, width: Int, hashA: Array[Long], topKActual: Int,
      topKInternal: Int, confidence: Double, eps: Double): TopKCMS[T] =
  ...
  def read(kryo: Kryo, input: Input): TopKHokusai[_] = {
    ...
    new TopKHokusai(cmsParams, windowSize,
      ...
      intervalTracker, t, Some(mBar))(classTag)
  }
