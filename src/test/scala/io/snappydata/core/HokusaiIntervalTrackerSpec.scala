...
import org.apache.spark.sql.execution.{IntervalTracker, Hokusai, CMSParams}
...
class HokusaiIntervalTrackerSpec extends FlatSpec with Matchers {
  ...
  /////////////////////////////////////////////////////////////////////////////
  "Hokusai interval conversion between last nth interval to interval from begining " should
      "be correct" in {
    val cmsParams = CMSParams(512, 7, SEED)
    val h = new Hokusai[Int](cmsParams, 1, 0, MutableList[CountMinSketch[Int]](),
        MutableList[CountMinSketch[Int]](), new IntervalTracker(), 0, None)
  ...
  "Hokusai Linked Interval" should "be correct after n intervals" in {
    ...
    {
      // After 3 interval , we should get best path of  2 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 4 interval , we should get best path of 1, 2 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 5 interval , we should get best path of  4 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 6 interval , we should get best path of  1,4 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 7 interval , we should get best path of  2,4 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 8 interval , we should get best path of  1,2,4 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 9 interval , we should get best path of  8 ( the implicit 1 interval will
      // always be added
    ...
    {
      // After 10 interval , we should get best path of 1, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(10)
      assert(length === 10)
      assert(seq.length === 2)
      assert(seq.containsSlice(Array(1, 8)))
      assert(h.queryTillLastNIntervals(10, 1).map { _.estimate } ===
          Some(computeSumOfAP(70, -7, 10)))
    ...
    {
      // After 11 interval , we should get best path of 2, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(11)
      assert(length === 11)
      assert(seq.length === 2)
      assert(seq.containsSlice(Array(2, 8)))
      assert(h.queryTillLastNIntervals(11, 1).map { _.estimate } ===
          Some(computeSumOfAP(77, -7, 11)))
    ...
    {
      // After 12 interval , we should get best path of 1,2, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(12)
      assert(length === 12)
      assert(seq.length === 3)
      assert(seq.containsSlice(Array(1, 2, 8)))
      assert(h.queryTillLastNIntervals(12, 1).map { _.estimate } ===
          Some(computeSumOfAP(84, -7, 12)))
    ...
    {
      // After 13 interval , we should get best path of 4, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(13)
      assert(length === 13)
      assert(seq.length === 2)
      assert(seq.containsSlice(Array(4, 8)))
      assert(h.queryTillLastNIntervals(13, 1).map { _.estimate } ===
          Some(computeSumOfAP(91, -7, 13)))
    ...
    {
      // After 14 interval , we should get best path of1, 4, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(14)
      assert(length === 14)
      assert(seq.length === 3)
      assert(seq.containsSlice(Array(1, 4, 8)))
      assert(h.queryTillLastNIntervals(14, 1).map { _.estimate } ===
          Some(computeSumOfAP(98, -7, 14)))
    ...
    {
      // After 15 interval , we should get best path of 2, 4, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(15)
      assert(length === 15)
      assert(seq.length === 3)
      assert(seq.containsSlice(Array(2, 4, 8)))
      assert(h.queryTillLastNIntervals(15, 1).map { _.estimate } ===
          Some(computeSumOfAP(105, -7, 15)))
      assert(h.queryTillLastNIntervals(2, 1).map { _.estimate } ===
          Some(computeSumOfAP(105, -7, 2)))
      assert(h.queryTillLastNIntervals(3, 1).map { _.estimate } ===
          Some(computeSumOfAP(105, -7, 3)))
      assert(h.queryTillLastNIntervals(7, 1).map { _.estimate } ===
          Some(computeSumOfAP(105, -7, 7)))
    }
    ...
    {
      // After 16 interval , we should get best path of 1, 2, 4, 8 ( the implicit 1 interval will
      // always be added
      val (seq, length) = h.taPlusIa.intervalTracker.identifyBestPath(16)
      assert(length === 16)
      assert(seq.length === 4)
      assert(seq.containsSlice(Array(1, 2, 4, 8)))
      assert(h.queryTillLastNIntervals(16, 1).map { _.estimate } ===
          Some(computeSumOfAP(112, -7, 16)))
      assert(h.queryTillLastNIntervals(2, 1).map { _.estimate } ===
          Some(computeSumOfAP(112, -7, 2)))
      assert(h.queryTillLastNIntervals(4, 1).map { _.estimate } ===
          Some(computeSumOfAP(112, -7, 4)))
      assert(h.queryTillLastNIntervals(8, 1).map { _.estimate } ===
          Some(computeSumOfAP(112, -7, 8)))
  ...
  ////////////////////////////////////////////////////////////////
  "Hokusai queries for n past intervals using interpolation for " + TEST_NUM_KEYS + " keys" should
      "be approximately correct " in {
    // 7, 14 ,  28, 35, 42, 49, 56, 63, 70, 77, 84, 91
            ...
            start until x foreach {
              i =>
                ...
                if (keyToQuery > 0) {
                  ...
                  // println("expected freq=" + exactFreq + ", hokusai count= " + freqFromQuery)
  ...
  /////////////////////////////////////////////////////////////////
  "Hokusai queries at a given time in past  using interpolation for keys" should
      "be approximately correct " in {
    // 7, 14 ,  28, 35, 42, 49, 56, 63, 70, 77, 84, 91
  ...
  /////////////////////////////////////////////////////////////////
  "Hokusai queries between two time intervals in past  using interpolation for keys" should
      "be approximately correct " in {
    // 7, 14 ,  28, 35, 42, 49, 56, 63, 70, 77, 84, 91
    val cmsParams = CMSParams(NumberUtils.nearestPowerOf2GE(8192), 5, SEED)
    val h = new Hokusai[Int](cmsParams, 10, 0, MutableList[CountMinSketch[Int]](),
        MutableList[CountMinSketch[Int]](), new IntervalTracker(), 0, None)
...

    {
      ...
      assert(h.queryBetweenTime(15, 113, 1).map { _.estimate } ===
          Some(14 + 21 + 28 + 35 + 42 + 49 + 56 + 63 + 70 + 77 + 84))
  ...
  /////////////////////////////////////////////////////////////////
  "Hokusai queries between two time intervals (generic test) in past " +
      "using interpolation for keys" should "be approximately correct " in {
    // 7, 14 ,  28, 35, 42, 49, 56, 63, 70, 77, 84, 91
    ...
    (1 to TEST_NUM_INTERVALS) foreach {
      k =>
        val count = 7 * k;
        // println("Interval number ="+k + " count added ="+ count)
        h.addEpochData(Map[Int, Long](1 -> count))
        h.increment()
    }
    // For each interval in past check some keys
