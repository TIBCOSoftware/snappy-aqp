 ...
 */
class BootStrapAccuracySuite extends SnappyFunSuite
with BeforeAndAfterAll{

  private val disabled = true
  val bootStrapTrials = 100 // default
  // TODO Change as needed
  val sampleRatio : Double = 0.005
  ...
  protected override def newSparkConf(addOn: (SparkConf) => SparkConf): SparkConf = {
    ...
    .set(io.snappydata.Property.ClosedFormEstimates.name, s"$isClosedForm")
 ...
 "WeatherDelay Int," +
  ...
  def setupData(): Unit = {
   ...
   // val hfile: String = getClass.getResource("/1995-2015_ParquetData").getPath
    ...
    snContext.sql("set spark.sql.shuffle.partitions=3")
   ...
   /*
    val sampleDataFrame = airlineDataFrame.stratifiedSample(Map(
      "qcs" -> "UniqueCarrier,YearI,MonthI", "fraction" -> 0.03,
      "strataReservoirSize" -> "50"))
    println(" data loaded into  sample table")
    sampleDataFrame.write.format("com.databricks.spark.csv").save(
      "/Users/ashahid/workspace/snappy/latest-snappy/temp/sample.csv")
    println(" saved 3% data as parquert") */
  ...
  def warmUp(): Unit = {
   ...
   resultMonth_Avg.collect()
    ...
    val y =
      snc.sql(s"SELECT COUNT(ArrDelay) as count, sum(ArrDelay)" +
        s" as sum, lower_bound(count), upper_bound(count)  FROM airline" +
          s" where $whereConditionY confidence " + confidence)
    y.collect()
    logInfo("warm up end")
    ...
    for (i <- 0 until numloop) {
      try {
       ...
       // resultMonth_Sum.queryExecution.executedPlan
        ...
        val sumOnMonth = rowMonth_sum(0).getLong(0)
      ...
      //  println("Time taken to prepare the plan q2= "+ (t - startTime))
        ...
        assertAnalysis()
        val sumOnYear = rowYear_sum(0).getLong(0)
        ...
        startTime = System.currentTimeMillis()
        val resultMonth_Count =
          snc.sql(s"SELECT COUNT(ArrDelay) as count, lower_bound(count)," +
            s" upper_bound(count)  FROM airline" +
              s" where $whereConditionM confidence " + confidence)
        ...
        startTime = System.currentTimeMillis()
        val x =
          snc.sql(s"SELECT COUNT(ArrDelay) as count, sum(ArrDelay) as sum," +
            s" lower_bound(count), upper_bound(count)  FROM airline" +
              s" where $whereConditionM confidence " + confidence)
        x.collect()
        assertAnalysis()
        ...
        startTime = System.currentTimeMillis()
        val z =
          snc.sql(s"SELECT avg(ArrDelay) as avg, sum(ArrDelay) as sum," +
            s" lower_bound(avg), upper_bound(avg)  FROM airline" +
              s" where $whereConditionY confidence " + confidence)
        z.collect()
        AssertAQPAnalysis.bootStrapAnalysis(this.snc)
        ...
        startTime = System.currentTimeMillis()
        val b =
          snc.sql(s" Select avg(ArrDelay) as x , absolute_error(x)," +
            s"relative_error(x) from airline " +
              s" confidence " + confidence)
        b.collect()
        assertAnalysis()
        ...
        startTime = System.currentTimeMillis()
        val c =
          snc.sql(s" Select count(ArrDelay) as x , absolute_error(x)," +
            s"relative_error(x) from airline " +
              s" confidence " + confidence)
        c.collect()
        assertAnalysis()
       ...
       // deleteSampleTable()
        ...
        if (i > 0 && (i == 5 || i == 10 || (i % 10 == 0))) {
          ...
          logInfo("")
          logInfo("SUM with condition On Year for [" + i + "] iteration =" + sumOnYear)
          ...
          logInfo("")
          logInfo("Avg with condition On Month for [" + i + "] iteration  = " +
        ...
        case e: Exception =>
          ...
          logInfo("================BootStrap LowerBound UpperBound with " + sampleRatio +
          ...
          logInfo("Average query execution time =" + cumulativeTime(0).toFloat / (i.toFloat + 1))
          logInfo("")
          ...
          logInfo("Average query execution time =" + cumulativeTime(1).toFloat / (i.toFloat + 1))
          logInfo("")
          ...
          logInfo("Average query execution time =" + cumulativeTime(2).toFloat / (i.toFloat + 1))
          logInfo("")
          ...
          logInfo("Average query execution time =" + cumulativeTime(3).toFloat / (i.toFloat + 1))
          logInfo("")
          ...
          logInfo("Average query execution time =" + cumulativeTime(4).toFloat / (i.toFloat + 1))
          logInfo("")
          ...
          logInfo("Average query execution time =" + cumulativeTime(5).toFloat / (i.toFloat + 1))
  ...
  ignore("BootStrap LowerBound and UpperBound over 1000 iterx") {
    this.testBounds()
  }
  ...
  ignore("test for Finding BootStrap SD") {
    ...
    logInfo("================BootStrap SD with " + sampleRatio +
  ...
  ignore("test null handling") {
    ...
    val totalSampleCnt = snc.sql("select count(*) as population_count," +
        " count(*) as sample_ from airlineSampled")
