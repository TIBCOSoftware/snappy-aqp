...
class BSAFunctionWithDatasourceAPITest extends BootStrapAggregateFunctionTest {
  ...
  def createTable(sqlContext: SQLContext,
    ...
    val ddlStr = "(" +
        ...
        "l_comment varchar(20)," +
        "scale Int" +
        ")"

    val df = if (isSample) {
      snc.sql(s"CREATE TABLE $tableName $ddlStr" +
        "USING column_sample " +
        "options " +
        "(" +
        "qcs 'l_quantity'," +
