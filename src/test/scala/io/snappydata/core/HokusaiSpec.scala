...
import org.apache.spark.sql.execution.{CMSParams, Hokusai, IntervalTracker}
...
*/
  ...
  "Hokusai TimeAggregation" should "be correct after n epochs" in {
    val h = new Hokusai[Long](cmsParams, 1, 0, MutableList[CountMinSketch[Long]](),
        MutableList[CountMinSketch[Long]](), new IntervalTracker(), 0, None)
    ...
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(5)) // m(0): [0,1)
    (1 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 2nd Epoch (1):  total num 1s seen: 10
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(10)) // m(1): [0,2)  // 5
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(5)) // m(0): [2,3)  // 10
    (2 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 3rd Epoch (2):  total num 1s seen: 15
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(15)) // m(1): [0,2)  // 5
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(10)) // m(1): [0,2)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(5)) // m(0): [2,3)  // None
    (3 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 4th Epoch (3):  total num 1s seen: 20
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(20)) // m(2): [0,4)
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(15)) // m(2): [0,4)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(10)) // m(1): [2,4)
    h.queryTillTime(3, 1L).map { _.estimate } should be(Some(5)) // m(0): [3,4)
    (4 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 5th Epoch (4):  total num 1s seen: 25
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(25)) // m(2): [0,4)
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(20)) // m(2): [0,4)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(15)) // m(1): [2,4)
    h.queryTillTime(3, 1L).map { _.estimate } should be(Some(10)) // m(1): [2,4)
    h.queryTillTime(4, 1L).map { _.estimate } should be(Some(5)) // m(0): [4,5)
    (5 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 6th Epoch (5):  total num 1s seen: 30
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(30)) // m(2): [0,4)
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(25)) // m(2): [0,4)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(20)) // m(2): [0,4)
    h.queryTillTime(3, 1L).map { _.estimate } should be(Some(15)) // m(2): [0,4)
    h.queryTillTime(4, 1L).map { _.estimate } should be(Some(10)) // m(1): [4,6)
    h.queryTillTime(5, 1L).map { _.estimate } should be(Some(5)) // m(0): [5,6)
    (6 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 7th Epoch (6):  total num 1s seen: 35
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(35)) // m(2): [0,4)
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(30)) // m(2): [0,4)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(25)) // m(2): [0,4)
    h.queryTillTime(3, 1L).map { _.estimate } should be(Some(20)) // m(2): [0,4)
    h.queryTillTime(4, 1L).map { _.estimate } should be(Some(15)) // m(1): [4,6)
    h.queryTillTime(5, 1L).map { _.estimate } should be(Some(10)) // m(1): [4,6)
    h.queryTillTime(6, 1L).map { _.estimate } should be(Some(5)) // m(0): [6,7)
    (7 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 8th Epoch (7):  total num 1s seen: 40
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(40)) // m(2): [0,8)
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(35)) // m(2): [0,8)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(30)) // m(2): [0,8)
    h.queryTillTime(3, 1L).map { _.estimate } should be(Some(25)) // m(2): [0,8)
    h.queryTillTime(4, 1L).map { _.estimate } should be(Some(20)) // m(2): [4,8)
    h.queryTillTime(5, 1L).map { _.estimate } should be(Some(15)) // m(2): [4,8)
    h.queryTillTime(6, 1L).map { _.estimate } should be(Some(10)) // m(1): [6,8)
    h.queryTillTime(7, 1L).map { _.estimate } should be(Some(5)) // m(0): [7,8)
    (8 until MAX_TIME) foreach { h.queryTillTime(_, 1L) should be(None) }

    // 9th Epoch (8):  total num 1s seen: 45
    h.addEpochData(FIVE_ONES)
    h.increment()
    h.queryTillTime(0, 1L).map { _.estimate } should be(Some(45)) // m(2): [0,8)
    h.queryTillTime(1, 1L).map { _.estimate } should be(Some(40)) // m(2): [0,8)
    h.queryTillTime(2, 1L).map { _.estimate } should be(Some(35)) // m(2): [0,8)
    h.queryTillTime(3, 1L).map { _.estimate } should be(Some(30)) // m(2): [0,8)
    h.queryTillTime(4, 1L).map { _.estimate } should be(Some(25)) // m(2): [4,8)
    h.queryTillTime(5, 1L).map { _.estimate } should be(Some(20)) // m(2): [4,8)
    h.queryTillTime(6, 1L).map { _.estimate } should be(Some(15)) // m(1): [6,8)
    h.queryTillTime(7, 1L).map { _.estimate } should be(Some(10)) // m(1): [6,8)
    h.queryTillTime(8, 1L).map { _.estimate } should be(Some(5)) // m(0): [8,9)
    ...
    (0 until 8) foreach { j =>
      (2L until 50L) foreach { n => h.queryTillTime(j, n).map { _.estimate } should be(Some(0)) }
    }
